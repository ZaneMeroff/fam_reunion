export const resumeData = {
  rates: [
    {
      name: "Hilton Phoenix Resort at the Peak",
      arrival: "Arrive: April 12, 2024",
      departure: "Depart: April 14, 2024",
      bullets: [
        "$249 S/D + tax for one-bedroom suite with single king or two queen beds with pullout sofa",
        "$25 (reduced from $35) + tax nightly resort fee (wi-fi, parking, fitness center, pools, water park, mini-golf)",
        "$9 one-time porterage and $2 nightly housekeeping service charge",
        "total cost for two nights: $XXX.XX",
        "to reserve call XXX.XXX.XXXX and say you're with the \"Nemoff Family Reunion\" April 12-14, 2024"
      ]
    },
  ],
  plans: [
    {
      name: "Friday",
      location: "April 12, 2024",
      time: "scheduled activities will be posted here closer to the event...",
      description: []
    },
    {
      name: "Saturday",
      location: "April 13, 2024",
      time: "scheduled activities will be posted here closer to the event...",
      description: []
    },
    {
      name: "Sunday",
      location: "April 14, 2024",
      time: "scheduled activities will be posted here closer to the event...",
      description: []
    },
  ],
  amenities: [
    {
      list: [
        "4 pools including a lazy river for kids of all ages",
        "River Ranch Water Park access included with room reservation",
        "gym and health club",
        "full service spa",
        "horseback riding in Dreamy Draw Park",
        "easy walk to and around the north Phoenix mountains",
        "for those in training, a walk (or run) up Piestewa Peak",
        "new, innovative Mexican food and fusion",
        "golf and putting greens",
        "BEST OF ALL: quality time with cool cousins and extended fam!",
      ]
    }
  ]
}
